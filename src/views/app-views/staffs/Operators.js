...
const Operators =({ staffUpdate, deleteStaff, editStaff, token }) => {
  ...
  const getOperators = async (page) => {
    setLoading(true)

    const res = await fetchStaffs(token, OPERATOR_GROUP, page)
    ...
    if (res.results) {
      data = res.results.map( (operator, index) => {
        console.log(operator.profile.image)
        return {
          key: index,
          id: operator.id,
          image: operator.profile ? operator.profile.image : null,
          name: operator.firstname + ' ' + (operator.lastname ? operator.lastname : ''),
          school: operator.school_name,
  ...
  return (
    <>
    {
      ...
      : <>
        <Table columns={columns} dataSource={operators} pagination={false} />
        {
          total > DATA_PER_PAGE ?
          <Flex justifyContent="end" className="mt-3">
            <Pagination current={currentPage} onChange={getOperators} total={total} pageSize={DATA_PER_PAGE}/>
          </Flex>
          : null
        }
      </>
