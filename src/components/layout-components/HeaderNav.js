...
export const HeaderNav = props => {
  const user_firstname = useSelector((state) => state.userLogin.user.firstname);
  const user_lastname = useSelector((state) => state.userLogin.user.lastname);
      ...
      <div className={`app-header-wrapper ${isNavTop ? 'layout-top-nav' : ''}`}>
        ...
        <div className="nav" style={{width: `calc(100% - ${getNavWidth()})`}}>
          ...
          <div className="nav-right">
            <span style={{marginTop: 4}}>Hello, <strong>{user_firstname} {user_lastname ? user_lastname : ''}</strong>!</span>
            <NavProfile >
